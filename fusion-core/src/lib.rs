...
pub struct FusionCore {
    ...
    /// where N is a 3mx3 matrix rotating virtual frame 
    /// to the frame of sensor i
...
impl FusionCore {
    ...
    /// Populate Np and T fields of struct
    ...
    fn compute_y(&self,n: &SMatrix<f64, THREE_N,3>, p: &SMatrix<f64,THREE_N,1>) -> SMatrix<f64, THREE_N, 3> {
        ...
        for i in 0..N_IMUS {
            ...
            let ss_pi: Matrix3<f64> = self.skew_symmetric(&pi);
            let product: Matrix3<f64> = ri*ss_pi;
    ...
    fn skew_symmetric(&self, v: &SMatrix<f64,3,1>) -> Matrix3<f64> {
    Matrix3::new(
         0.0, -v[2],  v[1],
        v[2],  0.0, -v[0],
       -v[1],  v[0],  0.0,
